...
function periodData(scheduleObj) {
	...
	for (let i = 0; i < schedule.length; i++) {
		if (now >= schedule[i].startDate && now <= schedule[i].endDate) return {
		...
		if (now >= schedule[i].endDate) {
			...
			if (now <= schedule[i+1].startDate) return {
			...
			<div style={{display: "flex", width: "100%"}}>
				...
				<div style={{flexGrow: 1}} className={classes.padding}>
					<div className={classes.leftRightPadding}>
						<Typography align="center" className={classes.bold}>{period.name}</Typography>
						<Grid container>
							<Grid item xs={6}>
